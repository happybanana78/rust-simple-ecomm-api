use crate::cart::user_cart::handler;
use crate::middlewares::auth::AuthMiddleware;
use actix_web::web::{ServiceConfig, delete, post, put, resource, scope};

pub fn routes(cfg: &mut ServiceConfig) {
    cfg.service(
        scope("/user")
            .wrap(AuthMiddleware::new(None))
            .service(resource("/get").route(post().to(handler::get_user_cart)))
